...
def register_user(phone=None, email=None, real_name="测试用户", user_type="设计师",
                  auth_type="个人", company="测试公司", role=None):
    """注册用户并自动登录"""
    ...
    if user_id:
        ...
        if admin_token:
            requests.post(API(f"/api/admin/users/{user_id}/approve"), headers=get_headers(admin_token))
...
def test_register_phone():
    """测试手机号注册"""
    print_step("测试手机号注册")
    # 甲方用户需要使用业主、建设单位等 user_type
    user = register_user(real_name="手机注册测试", user_type="业主", auth_type="个人", role="甲方")
...
def test_register_email():
    """测试邮箱注册"""
    print_step("测试邮箱注册")
    user = register_user(email=rand_email(), real_name="邮箱注册测试", user_type="建设单位", auth_type="个人", role="甲方")
    if user and user.get("token"):
        test_data["users"].append(user)
...
def test_register_enterprise():
    """测试企业注册"""
    print_step("测试企业注册")
    user = register_user(real_name="企业测试", user_type="建设单位", auth_type="企业", company="测试企业公司", role="甲方")
    if user and user.get("token"):
        test_data["users"].append(user)
...
def test_get_me():
    ...
    if not token:
        # 创建一个新甲方用户
        user = register_user(real_name="测试用户", user_type="业主", auth_type="个人", role="甲方")
        if user:
            test_data["users"].append(user)
            test_data["tokens"]["phone_user"] = user["token"]
            token = user["token"]
...
def test_create_quote():
    ...
    # 创建一个企业用户作为供方（乙方）
    seller = register_user(real_name="供方测试", user_type="设计院", auth_type="企业", company="供方公司", role="乙方")
    if not seller:
...
def test_select_winner():
    ...
    success = resp.status_code == 200
    print_result("选择中标", success, resp.text if not success else "")
    
    # 选择中标后，订单会自动创建，从订单列表获取订单ID
    if success:
        orders_resp = requests.get(API("/api/orders"), headers=get_headers(buyer_token))
        if orders_resp.status_code == 200:
            orders = orders_resp.json()
            if orders:
                test_data["ids"]["order_id"] = orders[0]["id"]
    
    return success


def test_create_order():
    """测试获取自动创建的订单（选择中标后订单自动创建）"""
    print_step("测试获取订单")
    buyer_token = test_data["tokens"].get("phone_user")
    demand_id = test_data["ids"].get("demand_id")

    if not buyer_token or not demand_id:
        print_result("获取订单", False, "缺少参数")
        return None
    ...
    if orders_resp.status_code == 200:
        ...
        # 找到对应需求的订单
        for order in orders:
            if order.get("demand_id") == demand_id or order.get("title", "").find(str(demand_id)) >= 0:
                test_data["ids"]["order_id"] = order["id"]
                print_result("获取订单", True)
                return order
        # 如果没找到对应需求，返回第一个订单
        if orders:
            test_data["ids"]["order_id"] = orders[0]["id"]
            print_result("获取订单", True, "（未找到对应需求，使用最新订单）")
            return orders[0]
    print_result("获取订单", False, orders_resp.text if orders_resp.status_code != 200 else "无订单")
    return None
